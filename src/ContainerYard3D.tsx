...
function ContainerTooltip({ slot, block }: { slot: Slot; block: keyof typeof blockColors }) {
  return (
    <div style={{
      ...
      <div style={{ marginBottom: '8px', fontWeight: 'bold', fontSize: '14px', color: blockColors[block] }}>
        Block {block} - Container Details
      </div>
      <div><strong>Container Code:</strong> {slot.kode_kontainer || 'N/A'}</div>
      <div><strong>Position:</strong> Row {slot.row}, Column {slot.column}, Tier {slot.tier}</div>
      <div><strong>Vessel/Voyage:</strong> {slot.vessel_voy || 'N/A'}</div>
      <div><strong>Size:</strong> {slot.size || 'N/A'}</div>
      <div><strong>Grade:</strong> {slot.grade || 'N/A'}</div>
      <div><strong>Status:</strong> <span style={{ 
        color: slot.status === 'occupied' ? '#4caf50' : '#ff9800',
        fontWeight: 'bold'
      }}>{slot.status}</span></div>
    </div>
  );
}
    ...
    <group>
      ...
      >
        ...
        <meshStandardMaterial 
          color={hovered ? '#ffffff' : blockColors[block]}
          transparent={hovered}
...
export default function ContainerYard3D() {
  ...
  return (
    <Canvas
      camera={{ position: [centerX, 20, 25], fov: 50 }}
      style={{ width: "100vw", height: "100vh", background: "#e0e7ef" }}
    >
      ...
      <Suspense fallback={null}>
        {yard.blocks.map((block) =>
          block.slots.map((slot, idx) => (
            <ContainerBox
              key={block.block_name + idx}
              slot={slot}
              block={block.block_name as keyof typeof blockColors}
            />
          ))
        )}
        <OrbitControls
